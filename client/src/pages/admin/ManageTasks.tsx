...
const ManageTasks: React.FC = () => {
  ...
  const {
    ...
    isLoading,
    isError,
  } = useQuery<TaskWithClient[], Error>("adminTasks", async () => {
    const response = await adminApi.getTasks();
    if (!response.data.success || !response.data.data) {
      throw new Error(
        response.data.error?.toString() || "Failed to fetch tasks"
      );
    }
  ...
  const handleDeleteTask = (taskId: string) => {
    if (
      window.confirm(
        "Are you sure you want to permanently delete this task and all its related data? This action cannot be undone."
      )
    ) {
